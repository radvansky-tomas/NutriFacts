...
// MARK: Radvansky - StickyHeaderViewDelegate
// This custom Header delegate extends Persei to provide basic information about reveal status
public protocol StickyHeaderViewDelegate {
    func stickyHeaderViewDelegateWillShow(header: StickyHeaderView) -> Void
    func stickyHeaderViewWillHide(header: StickyHeaderView) -> Void
    func stickyHeaderViewDidShow(header: StickyHeaderView) -> Void
    func stickyHeaderViewDidHide(header: StickyHeaderView) -> Void
}
...
public class StickyHeaderView: UIView {
    public weak var headerDelegate: StickyHeaderViewDelegate?
    // MARK: - Init
    func commonInit() {
    ...
    private func setRevealed(revealed: Bool, animated: Bool, adjustContentOffset adjust: Bool) {
        if animated {
            if revealed
            {
                headerDelegate?.stickyHeaderViewDelegateWillShow(self)
            }
            else
            {
                headerDelegate?.stickyHeaderViewWillHide(self)
            }
            UIView.animateWithDuration(0.2, delay: 0, options: .BeginFromCurrentState | .CurveEaseInOut, animations: {
                self.revealed = revealed
                    ...
                    if revealed
                    {
                        self.headerDelegate?.stickyHeaderViewDidShow(self)
                    }
                    else
                    {
                        self.headerDelegate?.stickyHeaderViewDidHide(self)
                    }
            })
        } else {
            self.revealed = revealed
            ...
            {
                headerDelegate?.stickyHeaderViewDelegateWillShow(self)
                headerDelegate?.stickyHeaderViewDidShow(self)
            }
            else
            {
                headerDelegate?.stickyHeaderViewWillHide(self)
                headerDelegate?.stickyHeaderViewDidHide(self)
            }
        }
    }
